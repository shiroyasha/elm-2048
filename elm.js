...
Elm.Main.make = function (_elm) {
   ...
   var main = function () {
      ...
      var model = defaultGrid(config.grid.rows);
...
Elm.Views.Grid.make = function (_elm) {
   ...
   var cellPosition = F3(function (config,
   location,
   _v0) {
      return function () {
         return function () {
            var sizeWithPadding = $Basics.toFloat(config.cell.size + 2 * config.cell.padding);
            var x = ($Basics.toFloat($Matrix.row(location)) + 1 / 2) * sizeWithPadding;
            var y = ($Basics.toFloat($Matrix.col(location)) + 1 / 2) * sizeWithPadding;
   ...
   model) {
      return $Matrix.flatten(A2($Matrix.mapWithLocation,
      cellPosition(config),
      model));
   });
